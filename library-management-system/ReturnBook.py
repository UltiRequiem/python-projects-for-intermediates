...
def returnn():
    ...
    try:
        ...
        if bid in allBid:
            checkAvail = (
                "select status from " + bookTable + " where bid = '" + bid + "'"
            )
    ...
    print(status)
    updateStatus = (
        "update " + bookTable + " set status = 'avail' where bid = '" + bid + "'"
    )
    try:
...
def returnBook():
    ...
    headingLabel = Label(
        headingFrame1, text="Return Book", bg="black", fg="white", font=("Courier", 15)
    )
